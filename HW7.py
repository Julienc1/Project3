...
table_spec = 'CREATE TABLE IF NOT EXISTS Tweets (tweet_id INTEGER, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)'
...
tweet_d = json.dumps(umsi_tweets, indent=2)
...
for tweet in umsi_tweets:
	cur.execute('INSERT INTO Tweets VALUES (?, ?, ?, ?, ?)',
        (tweet["id"], tweet["user"]["screen_name"], tweet["user"]["created_at"], tweet["text"], tweet["retweet_count"]))
conn.commit()
