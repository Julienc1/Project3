...
statement = 'CREATE TABLE IF NOT EXISTS Tweets (tweet_id TEXT PRIMARY KEY, tweet_text TEXT, user_id REFERENCES Users(user_id), time_posted TIMESTAMP, retweets INTEGER)'
cur.execute(statement)
...
for person in mentions:
	user_information= get_user_tweets(person)
...
for tweet in umich_tweets:
	cur.execute('INSERT OR IGNORE INTO Tweets VALUES (?, ?, ?, ?, ?)',
        (tweet["id_str"], tweet["text"], tweet["user"]["id_str"], tweet["user"]["created_at"], tweet["retweet_count"]))
conn.commit()
...
cur.execute(query)
screen_names1 = cur.fetchall()
screen_names = [" ".join(x) for x in screen_names1]
